...
function onDrop(ev: DragEvent){
  ...
  emit('reorder', dragged, props.node.id)
}
</script>
    ...
    >
      ...
      <template v-if="!editing">
        ...
        <div class="ml-auto flex items-center gap-2">
          ...
          <button class="btn-ghost" @click="$emit('add-child', node.id, node.depth)" :disabled="!canAddChild" title="Добавить подкатегорию">➕</button>
        </div>
      </template>
